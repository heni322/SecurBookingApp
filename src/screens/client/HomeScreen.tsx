    ...
    <View style={styles.screenWrapper}>
      ...
      >
        ...
        {loading && !missions ? (
          ...
          <View style={styles.statsRow}>
            {/* All three cards share the same standard surface background,
                all icons use colors.primary (gold) */}
            <StatCard Icon={Shield}      value={totalMissions}   label={t('stats.total')}       />
            <StatCard Icon={CheckCircle} value={completedCount}  label={t('stats.completed')}   />
            <StatCard Icon={RefreshCw}   value={inProgressCount} label={t('stats.in_progress')} />
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  newMissionBtn:        { width: 48, height: 48, borderRadius: 24, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', shadowColor: colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 6 },
  statsRow:             { flexDirection: 'row', gap: spacing[3], marginBottom: spacing[5] },
  ctaBanner:            { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.primary, borderRadius: radius['2xl'], padding: spacing[5], marginBottom: spacing[5], overflow: 'hidden', shadowColor: colors.primary, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 16, elevation: 8 },
  ctaDots:              { position: 'absolute', right: 24, flexDirection: 'row', gap: spacing[2] },
  ctaDot:               { width: 6, height: 6, borderRadius: 3, backgroundColor: palette.white },
...
const sosStyles = StyleSheet.create({
  overlay:     { flex: 1, backgroundColor: colors.scrim, alignItems: 'center', justifyContent: 'flex-end' },
  ...
  sendBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing[2], width: '100%', paddingVertical: spacing[4], backgroundColor: colors.dangerSurface, borderRadius: radius.full, shadowColor: colors.dangerSurface, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 6 },
